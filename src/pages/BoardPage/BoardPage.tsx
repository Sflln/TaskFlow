...
export const BoardPage = () => {
  ...
  if (!board) return <div className="board-page">Board not found — <Link to="/dashboard">back</Link></div>
  ...
  return (
    <div className="board-page">
      ...
      <div className="add-column-wrap">
        <input placeholder="New column name" value={newColName} onChange={(e) => setNewColName(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleAddColumn()} />
        ...
        {board.columns.map((col) => (
          <div key={col.id} className={`column ${dragOverCol === col.id ? 'drop-zone-active' : ''}`} onDrop={(e) => onDrop(e, col.id)} onDragOver={(e) => onDragOver(e, col.id)} onDragLeave={onDragLeave}>
            <div className="column-header">
              <h4>{col.name}</h4>
              <button className="column-delete-btn" onClick={() => deleteColumn(board.id, col.id)} title="Delete column">×</button>
            </div>
                ...
                return (
                  <div key={tid} className="task-card" draggable onDragStart={(e) => onDragStart(e, tid, col.id)}>
                    ...
                    <div className="task-assignee">
                      ...
                      <button className="task-delete-btn" onClick={() => deleteTask(board.id, tid)} title="Delete task">×</button>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="add-task-form">
              <input placeholder="New task title" value={selectedColumn === col.id ? newTaskTitle : ''} onChange={(e) => { setSelectedColumn(col.id); setNewTaskTitle(e.target.value) }} onKeyPress={(e) => e.key === 'Enter' && handleAddTask(col.id)} />
              <button onClick={() => handleAddTask(col.id)}>Add task</button>
            </div>
          </div>
        ))}
      </div>
